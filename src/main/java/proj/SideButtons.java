...
public class SideButtons { // a class for side buttons
    ...
    private void initializeSideButtons() { // a method to initialize side buttons
        home = new Button();
        home.setGraphic(new ImageView(Main.class.getResource("/home.png").toExternalForm()));

        add = new Button();
        add.setGraphic(new ImageView(Main.class.getResource("/add.png").toExternalForm()));

        load = new Button();
        load.setGraphic(new ImageView(Main.class.getResource("/load.png").toExternalForm()));

        save = new Button();
        save.setGraphic(new ImageView(Main.class.getResource("/save.png").toExternalForm()));

        save = new Button();
        save.setGraphic(new ImageView(Main.class.getResource("/save.png").toExternalForm()));

        info = new Button();
        info.setGraphic(new ImageView(Main.class.getResource("/info.png").toExternalForm()));

        clear = new Button();
        clear.setGraphic(new ImageView(Main.class.getResource("/clear.png").toExternalForm()));

        exit = new Button();
        exit.setGraphic(new ImageView(Main.class.getResource("/exit.png").toExternalForm()));
    ...
    private void actions() { // a method to handle all action
        ...
        load.setOnAction(e -> { // load data from files
            ...
            fileChooser.setTitle("Load movies file");
            fileChooser.getExtensionFilters().add(
                    new FileChooser.ExtensionFilter("Text files", "*.txt")
            );

            File selectedFile = fileChooser.showOpenDialog(stage);
            if (selectedFile != null && MovieCatalog.loadMoviesFromFile(selectedFile.toPath()))
